...
struct
    ...
    include ListOf_base
    include Datatype_of(ListOf_base)
end

module Altern1 (T:DATATYPE) :
    DATATYPE with type t = T.t =
struct
    module Altern1_base = struct
        type t = T.t
        let equal = T.equal
    ...
    include Altern1_base
    include Datatype_of(Altern1_base)
end
...
struct
    ...
    include Altern2_base
    include Datatype_of(Altern2_base)
end

module Option (T:DATATYPE) :
    DATATYPE with type t = T.t option =
struct
    module Option_base = struct
        type t = T.t option
        let equal a b = match a,b with
    ...
    include Option_base
    include Datatype_of(Option_base)
end
